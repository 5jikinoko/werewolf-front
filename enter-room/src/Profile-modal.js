...
const IconSelectModal = (props) => {
    ...
    useEffect(async () => {
        ...
        if (roomID !== null) {
            fetch("http://160.16.141.77:51000/icon-list?roomID=" + roomID, {
                method: "GET",
            })
            .then(response => response.json())
            .then(json => {
                console.log(json);
                console.log(json.iconList);
                setIconList(json.iconList);
            })
            .catch((error) => {
                console.error(error);
            });
...
export default function ProfileModal() {
    ...
    useEffect(() => {
        ...
        const roomID = getParam("roomID");
        if (roomID !== null) {
            fetch("http://160.16.141.77:51000/used-name?roomID=" + roomID, {
                method: "GET",
            })
            .then(response => response.json()) 
            .then(json => {
                console.log(json);
                console.log(json.usedNameList);
                usedNameList = json.usedNameList;
            })
            .catch((error) => {
                console.error(error);
            });
    ...
    const registerProfile = () => {
        ...
        //他のプレイヤーと名前の被りがある
        if (usedNameList.find(usedName => userName === userName)) {
            alert("他のプレイヤーとプレイヤーネームが被っています");
            return;
        }
        ...
        const roomID = getParam("roomID");
        fetch("http://160.16.141.77:51000/profile-register?announce=false&checkDuplicates=" + roomID, {
            method: "PUT",
            body: fd
        })
        .then((response) => {
    ...
    return(
        <div className="row justify-content-end">
            <div className="my-box col-auto card bg-light" onClick={() => setIsOpen(true)}>
                <img className="container-fluid" src={"http://160.16.141.77:51000/" + icon + ".png"}/>
                <div>プロフィールを編集する</div>
            </div>
